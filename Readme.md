...
+ Creating an object that describes the data structure of each worksheet that can then be passes to a pandas program that will use the information to create a DataFrame that includes all relevant data on each sheet.
...
#==============
In order to use this module download the WorkbookFunctions.py file. You will need to add this file to a folder in the python path. To determine what folders are in the python path open up a DataNitro iPython shell and do the following:
...
```python
compile_result = compile.compile_sheets(file_list, 'compiled_workbook.xls', 'Sewing', 'Summary')
compile_result
```
...
If the user does not supply a date_dict argument then the method will create one by calling the `check_all_dates()` method.
...
Again, if the user does not supply a date_dict argument then the method will create one by calling the `check_all_dates()` method.
...
> An exception will be raised if it is not the case that all elements of the list passed are not integers. 
